...
async def delete_quiz(quiz_id: int, db: Session = Depends(get_db)):
    ...
    # Safely delete dependent relations manually to avoid IntegrityErrors
    from database import GameSession, Question, Player, Answer
    
    sessions = db.query(GameSession).filter(GameSession.quiz_id == quiz_id).all()
    for session in sessions:
        players = db.query(Player).filter(Player.game_session_id == session.id).all()
        for player in players:
            db.query(Answer).filter(Answer.player_id == player.id).delete(synchronize_session=False)
        db.query(Player).filter(Player.game_session_id == session.id).delete(synchronize_session=False)
    db.query(GameSession).filter(GameSession.quiz_id == quiz_id).delete(synchronize_session=False)
