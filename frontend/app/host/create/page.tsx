  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const generatedQuestions = (response.data.questions || []).map((q: Question) => ({
        ...q,
        time_limit: q.question_type === 'code' ? 300 : defaultTimeLimit,
        options: q.options || [],
      }))
            ...
            >
              <div className="flex items-start justify-between mb-4">
                <span className="text-sm font-mono text-white/60">
                  Question {index + 1} {q.question_type === 'code' ? '(Coding Problem)' : '(Multiple Choice)'}
                </span>
              ...
              {editingIndex === index ? (
                <div className="space-y-3">
                  <textarea
                    ...
                    rows={2}
                  />
                  {q.question_type !== 'code' && q.options && q.options.map((opt, optIndex) => (
                    <input
                      key={optIndex}
                      type="text"
                      value={opt}
                  ...
                  {q.question_type !== 'code' && q.options && (
                    <select
                  ...
                  {q.question_type === 'code' && q.test_cases && (
                    <div className="space-y-3">
                      {q.boilerplate_code && (
                        <div className="bg-black/40 p-3 rounded-xl text-sm font-mono text-white/70 overflow-auto max-h-32">
                          <p className="mb-2 font-semibold text-white/90">Boilerplate Code:</p>
                          <pre className="whitespace-pre-wrap">{q.boilerplate_code}</pre>
                        </div>
                      )}
                      <div className="bg-black/40 p-4 rounded-xl text-sm font-mono text-white/70 overflow-auto max-h-32">
                        <p className="mb-2 font-semibold text-white/90">Test Cases ({q.test_cases.length} total):</p>
                        {q.test_cases.map((tc, tcId) => (
                          <div key={tcId} className="mb-2 last:mb-0 border-l-2 border-white/20 pl-2">
                            <div className="text-xs text-white/50 mb-1">{tc.is_sample ? '✓ Sample' : '⊕ Hidden'} Case {tcId + 1}</div>
                            <div><span className="text-white/40">In:</span> {tc.input}</div>
                            <div><span className="text-white/40">Out:</span> {tc.expected_output}</div>
                          </div>
                        ))}
                      </div>
                    </div>
                ...
                <div>
                  ...
                  {q.question_type === 'code' && q.test_cases && (
                     <div className="space-y-3">
                       {q.boilerplate_code && (
                         <div className="bg-white/5 p-3 rounded-lg text-sm font-mono text-white/70 overflow-auto max-h-24">
                           <span className="text-xs font-semibold text-blue-300">Boilerplate Code:</span>
                           <pre className="mt-1 whitespace-pre-wrap text-xs">{q.boilerplate_code}</pre>
                         </div>
                       )}
                       <div className="bg-white/5 p-3 rounded-lg">
                           <span className="text-sm font-semibold text-blue-300">Code Question</span>
                           <p className="text-xs text-white/50 mt-1">
                             {q.test_cases.filter((tc) => tc.is_sample).length} sample · {q.test_cases.filter((tc) => !tc.is_sample).length} hidden test cases
                           </p>
                       </div>
                     </div>
                  )}
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <input 
                      type="number"
                      className="input-field min-h-0 h-8 py-1 px-2 w-20 text-sm bg-white/10"
                      value={q.time_limit}
                      onChange={(e) => handleUpdateQuestion(index, 'time_limit', parseInt(e.target.value) || 15)}
                      min="1"
                      max="600"
                    />
